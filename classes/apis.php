...
class ShopgatePluginApi extends ShopgateObject implements ShopgatePluginApiInterface {
	...
	public function handleRequest (array $data = array()) {
		...
		try {
			...
			if (
				!$this->config->getIsShopgateAdapter() && !empty($this->params['shop_number'])
			...
			$errortext
				= ShopgateLibraryException::getMessageFor(ShopgateLibraryException::MERCHANT_API_ERROR_RECEIVED) . ': "' . $e->getCode()
				. ' - ' . $e->getMessage() . '"';
		} catch (Exception $e) {
			$message = "\n" . get_class($e) . "\n";
	...
	protected function addOrder () {
		...
		if (empty($orders)) {
			throw new ShopgateLibraryException(ShopgateLibraryException::MERCHANT_API_INVALID_RESPONSE,
	...
	protected function checkCart () {
		...
		if (!is_array($cartData)) {
			throw new ShopgateLibraryException(
				...
				'$cartData Is type of : ' . is_object($cartData)
					? get_class($cartData)
					: gettype($cartData)
			);
		}
		...
		if (!empty($cartData['shipping_methods'])) {
			foreach ($cartData["shipping_methods"] as $shippingMethod) {
				...
				if (!is_object($shippingMethod) || !($shippingMethod instanceof ShopgateShippingMethod)) {
					throw new ShopgateLibraryException(
						ShopgateLibraryException::PLUGIN_API_WRONG_RESPONSE_FORMAT,
						'$shippingMethod Is type of : ' . is_object($shippingMethod)
		...
		if (!empty($cartData['payment_methods'])) {
			foreach ($cartData["payment_methods"] as $paymentMethod) {
				...
				if (!is_object($paymentMethod) || !($paymentMethod instanceof ShopgatePaymentMethod)) {
					throw new ShopgateLibraryException(
						ShopgateLibraryException::PLUGIN_API_WRONG_RESPONSE_FORMAT,
						'$paymentMethod Is type of : ' . is_object($paymentMethod)
		...
		if (!empty($cartData['external_coupons'])) {
			foreach ($cartData["external_coupons"] as $coupon) {
				...
				if (!is_object($coupon) || !($coupon instanceof ShopgateExternalCoupon)) {
					throw new ShopgateLibraryException(
						ShopgateLibraryException::PLUGIN_API_WRONG_RESPONSE_FORMAT,
						'$coupon Is type of : ' . is_object($coupon) ? get_class($coupon) : gettype($coupon)
					);
				}
				$coupon = $coupon->toArray();
				unset($coupon["order_index"]);
	...
	protected function setSettings () {
		if (empty($this->params['shopgate_settings']) || !is_array($this->params['shopgate_settings'])) {
			throw new ShopgateLibraryException(ShopgateLibraryException::PLUGIN_API_NO_SHOPGATE_SETTINGS,
	...
	private function clearCache () {
		...
		if (!empty($errorFiles)) {
			throw new ShopgateLibraryException(ShopgateLibraryException::PLUGIN_FILE_DELETE_ERROR,
				"Cannot delete files (" . implode(', ', $errorFiles) . ")", true);
		}
...
class ShopgateMerchantApi extends ShopgateObject implements ShopgateMerchantApiInterface {
	...
	protected function getCurlOptArray ($override = array()) {
		...
		$opt[CURLOPT_USERAGENT]
			= 'ShopgatePlugin/' . (defined('SHOPGATE_PLUGIN_VERSION') ? SHOPGATE_PLUGIN_VERSION : 'called outside plugin');
	...
	public function cancelOrder (
		$orderNumber, $cancelCompleteOrder = false, $cancellationItems = array(), $cancelShipping = false, $cancellationNote = ''
	) {
		$request = array(
			'action' => 'cancel_order',
			'order_number' => $orderNumber,
